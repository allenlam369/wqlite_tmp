...
import common.Utils;
import model.RiverWater2;
import model.RiverWater2PK;

public class UpdateRiverWater2 implements UpdateDbInterface {
	static Connection con; // for mssql
	static private EntityManager em = EntityManagerUtil.getEntityManager(); // for postgres
	static String dbName = "river_water2";

	public static void main(String[] args) throws SQLException {
		UpdateRiverWater2 me = new UpdateRiverWater2();
		me.run();
	}
	...
	void run() throws SQLException {
		...
		int rows = incrementalUpdateFromMssql(con, em, sql);
...
//				String sql1 = "SELECT top 1 mdate FROM WPG.river_water2 order by mdate DESC";
		...
		try {
			if (con != null) {
				...
				while (rs.next()) {
					...
					RiverWater2 row = new RiverWater2();
					...
					RiverWater2PK id = new RiverWater2PK();
